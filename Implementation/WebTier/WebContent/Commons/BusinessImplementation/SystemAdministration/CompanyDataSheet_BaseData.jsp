...
<title>Company Base Data Sheet</title>
	...
	<form name="form" method="post" action="${artifactView.codeBase }">
		...
		<div class="readOnlyContainer">
			<div class="row">
				...
				<span class="formw"><input type="text" name="companyName" value="${artifactView.companyName}"></span>
			</div>	
			<div class="row">
				<c:set property="key" value="ui.companyds.basedata.tradeRegisterNumber" target="${i18Helper}"></c:set>
				<span class="label">${i18Helper.value}:</span>
				<span class="formw"><input type="text" name="tradeRegisterNumber" value="${artifactView.tradeRegisterNumber}"></span>
			</div>		
			<div class="row">
				<c:set property="key" value="ui.companyds.basedata.taxNumber" target="${i18Helper}"></c:set>
				<span class="label">${i18Helper.value}:</span>
				<span class="formw"><input type="text" name="taxNumber" value="${artifactView.taxNumber}"></span>
			</div>	
			<div class="row">
				<c:set property="key" value="ui.companyds.basedata.administrator" target="${i18Helper}"></c:set>
				<span class="label">${i18Helper.value}:</span>
				<span class="formw"><input type="text" name="administrator" value="${artifactView.administrator}"></span>
			</div>			
			<div class="row">
				<c:set property="key" value="ui.companyds.basedata.adminPhone" target="${i18Helper}"></c:set>
				<span class="label">${i18Helper.value}:</span>
				<span class="formw"><input type="text" name="adminPhone" value="${artifactView.adminPhone}"></span>
			</div>			
		</div>
		...
		<div class="readOnlyContainer">
			<fieldset style="width: 400px; border-color: white;" >
				...
				<div class="row">
					...
					<span class="formw">
						<select id="zipSelect" name="zipCode" onChange="setToDirty();">
							<option value="-1">---</option>
							<c:forEach var="zipCode" items="${artifactView.defaultSettlementsZipCodes}" varStatus="index">						
				...
				<div class="row">
					...
					<span class="label">${i18Helper.value}:</span>
					<span class="formw"><input type="text" name="telecomAddress" value="${artifactView.telecomAddress}"></span>
				</div>		
				<div class="row">
					<c:set property="key" value="ui.pds.webpagedata.webpage" target="${i18Helper}"></c:set>
					<span class="label">${i18Helper.value}:</span>
					<span class="formw"><input type="text" name="webPageAddress" value="${artifactView.webPageAddress}"></span>
				</div>
