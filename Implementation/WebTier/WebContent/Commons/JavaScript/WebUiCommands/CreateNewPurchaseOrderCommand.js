// CreateNewPurchaseOrderCommand.js

// CreateNewPurchaseOrderCommand
function CreateNewPurchaseOrderCommand( theController, theStylesPath ) {
	...
	// inherit from WebUiCommand to make this object a specialized command
  	...
  	//private constants
  	var WINDOW_TITLE = "CreateNewPurchaseOrder";
  	var PURCHASE_ORDER_NAME_LABEL = "ProductPurchaseOrderNameLabel";
  	var PURCHASE_ORDER_INPUT_ID = "ProductPurchaseOrderNameInputID";

	//private instance variables
	var self = this;
	var purchaseOrderNameInput;
	...
	function _SetUpFormElements () {
		var formRow = self.createFormRow(controller.getText(PURCHASE_ORDER_NAME_LABEL), "input", PURCHASE_ORDER_INPUT_ID);
		
		self.addElement(formRow);
			
		purchaseOrderNameInput = self.getElementInWindowById(PURCHASE_ORDER_INPUT_ID);
	...
	function _OnCreateClickHandler() {
		...
		commandLine += "&artifactTypeName=ProductPurchaseOrderDataSheet";
		commandLine += "&artifactName="+purchaseOrderNameInput.value;
		commandLine += "&catalogName="+purchaseOrderNameInput.value;
		...
		if (!xmlError) {
			...
			if (resultTag!=null) {
				...
				else
				{
					...
					controller.loadDocument("ProductPurchaseOrderDataSheet", purchaseOrderNameInput.value ,"&artifactId="+artifactId, "BaseData");
				}
