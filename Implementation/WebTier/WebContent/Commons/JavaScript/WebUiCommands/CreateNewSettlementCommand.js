// CreateNewSettlementCommand.js

// CreateNewSettlementCommand
function CreateNewSettlementCommand( theController, theStylesPath ) {
	...
	// inherit from WebUiCommand to make this object a specialized command
  	...
  	//private constants
  	var WINDOW_TITLE = "CreateNewSettlement";
  	var SETTLEMENT_LABEL = "SettlementLabel";
  	var SETTLEMENT_INPUT_ID = "SettlementInputID";
	...
	function _SetUpFormElements () {
		var formRow = self.createFormRow(controller.getText(SETTLEMENT_LABEL), "input", SETTLEMENT_INPUT_ID);
		self.addElement(formRow);
		settlementInput = self.getElementInWindowById(SETTLEMENT_INPUT_ID);
	...
	function _OnCreateClickHandler() {
		...
		commandLine += "&artifactTypeName=SettlementDataSheet";
		commandLine += "&artifactName="+settlementInput.value;
		commandLine += "&settlementName="+settlementInput.value;
		commandLine += "&artifactFolderName=SystemAdministration";

		//alert("processing: "+commandLine);
		var xmlError=false;
		try { xmlFile = new XmlResrouce( commandLine ); }
		...
		if (!xmlError) {
			...
			if (resultTag!=null) {
				...
				else
				{
					...
					controller.loadDocument("SettlementDataSheet",settlementInput.value,"&artifactId="+artifactId, "BaseData");
				}
