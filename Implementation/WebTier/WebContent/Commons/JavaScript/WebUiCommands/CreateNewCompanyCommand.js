...
function CreateNewCompanyCommand( theController, theStylesPath ) {
	...
	// inherit from WebUiCommand to make this object a specialized command
  	...
  	//private constants
  	var WINDOW_TITLE = "CreateNewCompany";
  	var COMPANY_NAME_LABEL = "CompanyNameLabel";
  	var COMPANY_NAME_INPUT_ID = "CompanyNameInputID";
	...
	function _SetUpFormElements () {
		var formRow = self.createFormRow(controller.getText(COMPANY_NAME_LABEL), "input", COMPANY_NAME_INPUT_ID);
	
		self.addElement(formRow);		
		companyNameInput = self.getElementInWindowById(COMPANY_NAME_INPUT_ID);
	...
	function _OnCreateClickHandler() {
		...
		commandLine += "&artifactTypeName=CompanyDataSheet";
		commandLine += "&artifactName="+escape(companyNameInput.value.replace(/ /g, '_'));
		commandLine += "&companyName="+escape(companyNameInput.value);
		...
		if (!xmlError) {
			...
			if (resultTag!=null) {
				...
				else
				{
					...
					controller.loadDocument("CompanyDataSheet",companyNameInput.value.replace(/ /g, '_'),"&artifactId="+artifactId, "BaseData");
				}
