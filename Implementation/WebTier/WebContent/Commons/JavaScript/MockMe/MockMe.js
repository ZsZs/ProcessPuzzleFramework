...
ArgumentsMatchingHash = Class.create(IdentityHash, {
	...
	_matchArrays : function(array, arrayWithMatchers, ignoreMatchers) {
		if (!arrayWithMatchers instanceof Array)
			return false;
		if (array.length != arrayWithMatchers.length)
			return false;
		for ( var index = 0; index < array.length; index += 1 ) {
			var val = array[index];
			var valOrMatcher = arrayWithMatchers[index];
			if (!this._matchValues(val, valOrMatcher, ignoreMatchers))
				return false;
		}
		return true;
	},
	...
	_matchValues : function(val, valOrMatcher, ignoreMatchers) {
		...
		if (val === valOrMatcher)
			return true;
		if (valOrMatcher && valOrMatcher.match
				&& (valOrMatcher.match instanceof Function)) {
			if (ignoreMatchers)
				return false;
	...
	_matchObjects : function(obj, objOrMatcher, ignoreMatchers) {
		if (obj.constructor !== objOrMatcher.constructor)
			return false;
		for (prop in obj) {
			if (!this._matchValues(obj[prop], objOrMatcher[prop],
					ignoreMatchers))
				return false;
		}
...
var Mocker = Class.create( {
	...
	_mock : function(something) {
		if (typeof (something) == 'object')
			return this.mockObject(something);
		if (typeof (something) == 'function')
			return this.mockClass(something);
	...
	_shouldBeIgnored : function(f) {
		if (Object.values(Function.prototype).include(f))
			return true;
		return false;
	},
	...
	mockFunction : function(container, original) {
		...
		if (container && original) {
			...
			f.propName = name;
			f.originalFromPrototype = this._isFunctionFromPrototype(f.original,
					container, f.propName);
		}
		f.mocker = this;
...
var MockedFunction = {
	...
	verify : function(checkCount) {
		...
		if (!checkCount(count)) {
			throw new MockMe.Exception('Verify call error', this.propName + '('
					+ $A(args).join(', ') + ')' + ' should be called '
					+ checkCount.toString() + ' times but was called ' + count
					+ ' times');
		}
	},
	...
	unmock : function() {
		if (!this.container || !this.original)
			return;
...
var isInstanceOf = function(constructor) {
	if (typeof (constructor) != 'function')
		throw new MockMe.Exception("Can only take constructors",
